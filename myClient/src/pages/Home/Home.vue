 ...
 import * as StringUtils from '../../utils/string-utils'
  export default {
    ...
    mounted() {
      ...
      this.$axios.get('/users/homes').then(function(res) {
        console.log(res);
        // console.log(res.data.articles[0].retweeted_status);
        if (res.data.error !== 0) {
          console.log('Get data error!')
          return;
        }
          that.weiboContent = res.data.articles  //微博的所有内容
//         let tempTopTip = res.data.articles.card_group[0]
...
//         }
        ...
        that.bottomIsLoading = true
        setTimeout(() => {
          //故意推迟，以显示加载动画效果
          that.topIsLoading = false
        }, 1000)
      })
      that.addScrollEvent()
    },
